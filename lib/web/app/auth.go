...
func (h *Handler) completeAppAuthExchange(w http.ResponseWriter, r *http.Request, p httprouter.Params) error {
	...
	setAppSessionCookie(w, req.CookieValue, 0)
	...
	// keep feature disabled until registration and refresh handlers are implemented
	const DBSCEnabled = false
...
func setAppSessionCookie(w http.ResponseWriter, value string, maxAge time.Duration) {
	cookie := &http.Cookie{
		Name:     CookieName,
		Value:    value,
		Path:     "/",
