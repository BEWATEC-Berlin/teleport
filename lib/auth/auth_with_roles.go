...
func (a *ServerWithRoles) SignDBSCChallenge(ctx context.Context, sessionID string) (string, error) {
	...
	// check to ensure the session exists before we sign the challenge.
	if _, err := a.authServer.GetAppSession(ctx, types.GetAppSessionRequest{SessionID: sessionID}); err != nil {
		return "", trace.Wrap(err)
	}
