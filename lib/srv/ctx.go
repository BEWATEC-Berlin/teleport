...
func (c *ServerContext) ConfigureCommand(extraFiles ...*os.File) (*CommandExecutor, error) {
	command, err := c.ExecCommand()
	if err != nil {
		return nil, trace.Wrap(err)
	}
	executor, err := ConfigureCommand(c.CancelContext(), c.Logger, c.srv.ChildLogConfig().Writer, command, c.ExecType, extraFiles...)
	if err == nil {
